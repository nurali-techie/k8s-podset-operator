...
import (
	...
	appv1alpha1 "github.com/nurali-techie/k8s-podset-operator/pkg/apis/app/v1alpha1"
...
func (r *ReconcilePodSet) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	// list pods are ups (running / pending)
	var ups []corev1.Pod
	upNames := []string{}
	for _, pod := range podList.Items {
		...
		if pod.Status.Phase == corev1.PodRunning || pod.Status.Phase == corev1.PodPending {
			ups = append(ups, pod)
			upNames = append(upNames, pod.ObjectMeta.Name)
		}
	}
	upLen := int32(len(ups))
	...
	if !reflect.DeepEqual(podSet.Status, status) {
		...
		if err = r.client.Update(context.TODO(), podSet); err != nil {
			return reconcile.Result{}, err
		}
	}

	// scale down pods
	if upLen > podSet.Spec.Size {
		reqLogger.Info("Scalling down pods", "Currently up", upLen, "Required size", podSet.Spec.Size)
		diff := upLen - podSet.Spec.Size
		dpods := ups[:diff]
		for _, pod := range dpods {
	...
	// scale up pods
	if upLen < podSet.Spec.Size {
		reqLogger.Info("Scalling up pods", "Currently up", upLen, "Required size", podSet.Spec.Size)
